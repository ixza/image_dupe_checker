//credits go to https://gist.github.com/Bibimaw/8873663
// i just added compare_image() resizer
...
double compare_image(const char * file1, const char * file2)
	{
		cv::Mat image1 = cv::imread(file1, IMREAD_GRAYSCALE);
		cv::Mat image2 = cv::imread(file2, IMREAD_GRAYSCALE);

    	if (!image1.data || !image2.data) {
    		    std::cerr << "Error: Could not open or find the image." << std::endl;
				return 0.0;
    	}
		resize(image1, image1, Size(1500, 1500));
		resize(image2, image2, Size(1500, 1500));
		//int block_size = std::gcd(image1.cols, image1.rows);
		int block_size = 150;
		return qm::compute_quality_metrics(image1, image2, block_size);
	}
